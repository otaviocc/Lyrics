...
impl ColorSpec {
    pub fn resolve(&self) -> Result<Color, ColorError> {
        match self {
            ...
            Self::Name(name) => literal(name).ok_or_else(|| ColorError::new(name, PALETTE_FORMS)),
        }
    }
    ...
    pub fn resolve_against(&self, palette: &Palette) -> Result<Color, ColorError> {
        match self {
            Self::Index(index) => Ok(Color::Indexed(*index)),
            Self::Name(name) if name == NO_COLOR => Err(ColorError::new(name, NONE_FORMS)),
            Self::Name(name) => palette
                .slot(name)
                .or_else(|| literal(name))
                .ok_or_else(|| ColorError::new(name, ELEMENT_FORMS)),
        }
    }

    #[must_use]
    pub fn removes_color(&self) -> bool {
        matches!(self, Self::Name(name) if name == NO_COLOR)
    }
}
...
fn rgb(hex: &str) -> Option<Color> {
    ...
    let (red, rest) = hex.split_at_checked(2)?;
    let (green, blue) = rest.split_at_checked(2)?;
...
impl Visitor<'_> for ColorSpecVisitor {
    ...
    fn visit_u64<E: de::Error>(self, value: u64) -> Result<Self::Value, E> {
        u8::try_from(value)
            .map(ColorSpec::Index)
            .map_err(|_| E::custom(format!("{value} is not an index 0-255")))
    }
...
mod tests {
    ...
    fn a_slot_is_a_color_only_for_an_element() {
        ...
        assert!(
            name("accent").resolve().is_err(),
            "a palette slot must not name another slot"
        );
    }
    ...
    fn an_element_still_takes_a_literal() {
        let palette = Palette::default();
        assert_eq!(
            name("#ff0000").resolve_against(&palette),
            Ok(Color::Rgb(0xFF, 0, 0))
        );
        assert_eq!(
            name("light_blue").resolve_against(&palette),
            Ok(Color::LightBlue)
        );
    }
    ...
    fn unreadable_values_are_errors_rather_than_guesses() {
        for value in [
            ...
            assert!(
                name(value).resolve().is_err(),
                "{value:?} resolved to a color"
            );
        }
    }
    ...
    fn the_error_says_what_was_expected() {
        let error = name("blurple").resolve().unwrap_err();
    ...
    fn a_background_can_be_removed_but_a_foreground_cannot() {
        let palette = Palette::default();
        assert!(name(NO_COLOR).removes_color());
        assert!(
            !name("reset").removes_color(),
            "reset is a color: the terminal's own"
        );
        let error = name(NO_COLOR).resolve_against(&palette).unwrap_err();
    ...
    fn toml_spells_a_color_as_a_string_or_an_integer() {
        ...
        let string: Holder = toml::from_str(r#"fg = "accent""#).unwrap();
        assert_eq!(string.fg, name("accent"));

        let integer: Holder = toml::from_str("fg = 208").unwrap();
        assert_eq!(integer.fg, ColorSpec::Index(208));

        assert!(
            toml::from_str::<Holder>("fg = 300").is_err(),
            "300 is not an 8-bit index"
        );
        assert!(
            toml::from_str::<Holder>("fg = -1").is_err(),
            "-1 is not an 8-bit index"
        );
    }
}
