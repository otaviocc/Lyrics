...
impl PaletteFile {
    #[must_use]
    pub const fn slots(&self) -> [(&'static str, Option<&ColorSpec>); 15] {
        [
...
mod tests {
    ...
    fn every_slot_merges_from_the_child_and_no_other_slot_moves() {
        let names: Vec<&str> = PaletteFile::default()
            .slots()
            .iter()
            .map(|(name, _)| *name)
            .collect();
        let every = names.iter().fold(String::new(), |mut acc, name| {
            use std::fmt::Write as _;
            let _ = writeln!(acc, "{name} = 1");
            acc
        });

        for name in &names {
            let base: PaletteFile = toml::from_str(&every).unwrap();
            let child: PaletteFile = toml::from_str(&format!("{name} = 208\n")).unwrap();

            let mut palette = Palette::default();
            for (slot, spec) in child.merge(base).slots() {
                let color = spec.unwrap().resolve().unwrap();
            ...
            assert_eq!(
                ...
                "{name} did not come from the child"
            );
            for other in names.iter().filter(|other| *other != name) {
                assert_eq!(
                    palette.slot(other),
                    Some(Color::Indexed(1)),
                    "{other} moved when {name} was merged"
                );
            }
        }
    }
